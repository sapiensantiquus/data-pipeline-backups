...
let "retain=$retain-1"
if sudo test -d /mnt/backups/$backup_path/$interval.$retain; then
  echo "sudo rm -rf /mnt/backups/$backup_path/$interval.$retain"
  sudo rm -rf /mnt/backups/$backup_path/$interval.$retain
fi


# Rotate all previous backups (except the first one), up one level
for x in `seq $retain -1 2`; do
  if sudo test -d /mnt/backups/$backup_path/$interval.$[$x-1]; then
    echo "sudo mv /mnt/backups/$backup_path/$interval.$[$x-1] /mnt/backups/$backup_path/$interval.$x"
    sudo mv /mnt/backups/$backup_path/$interval.$[$x-1] /mnt/backups/$backup_path/$interval.$x
  fi
done

# Copy first backup with hard links, then replace first backup with new backup
if sudo test -d /mnt/backups/$backup_path/$interval.0 ; then
  echo "sudo cp -al /mnt/backups/$backup_path/$interval.0 /mnt/backups/$backup_path/$interval.1"
  sudo cp -al /mnt/backups/$backup_path/$interval.0 /mnt/backups/$backup_path/$interval.1
fi
if [ ! -d /mnt/backups/$backup_path ]; then
  echo "sudo mkdir -p /mnt/backups/$backup_path"
  sudo mkdir -p /mnt/backups/$backup_path
  echo "sudo chmod 700 /mnt/backups/$backup_path"
  sudo chmod 700 /mnt/backups/$backup_path
fi
...
echo "sudo rsync -avh --stats --delete --numeric-ids --log-file=/tmp/efs-backup.log /backup/ /mnt/backups/$backup_path/$interval.0/"
sudo rsync -ah --stats --delete --numeric-ids --log-file=/tmp/efs-backup.log /backup/ /mnt/backups/$backup_path/$interval.0/
rsyncStatus=$?
echo "sudo cp /tmp/efs-backup.log /mnt/backups/efsbackup-logs/$backup_path-`date +%Y%m%d-%H%M`.log"
sudo cp /tmp/efs-backup.log /mnt/backups/efsbackup-logs/$backup_path-`date +%Y%m%d-%H%M`.log
echo "sudo touch /mnt/backups/$backup_path/$interval.0/"
sudo touch /mnt/backups/$backup_path/$interval.0/
exit $rsyncStatus
